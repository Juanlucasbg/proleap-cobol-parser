...
async function appShellVisible(page: Page): Promise<boolean> {
  const negocioVisible = await page.getByText(textRegex("Negocio")).first().isVisible().catch(() => false);
  const sidebarVisible = await page.locator("aside, nav").first().isVisible().catch(() => false);
  return negocioVisible && sidebarVisible;
}

async function openMiNegocioMenu(page: Page): Promise<void> {
  const rootNegocio = await firstVisibleByText(page, ["Negocio"], 6_000);
  if (rootNegocio) {
    await rootNegocio.click();
    await waitForUiToLoad(page);
  }

  const miNegocio = await firstVisibleByText(page, ["Mi Negocio"], 6_000);
  if (miNegocio) {
...
test("saleads_mi_negocio_full_test", async ({ page, context }, testInfo) => {
  ...
  await runSection("Login", async (result) => {
    ...
    const alreadyLoggedIn = await appShellVisible(appPage);
    if (alreadyLoggedIn) {
      result.details.push("Session already authenticated; dashboard is already visible.");
    } else {
      const popupPromise = context.waitForEvent("page", { timeout: 7_000 }).catch(() => null);
  ...
  await fs.writeFile(finalReportPath, finalReportJson, "utf8");
  await fs.writeFile(path.resolve(process.cwd(), "saleads-mi-negocio-final-report.json"), finalReportJson, "utf8");
