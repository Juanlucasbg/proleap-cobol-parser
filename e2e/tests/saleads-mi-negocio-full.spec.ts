...
async function openLegalLinkAndResolvePage(appPage: Page, linkText: RegExp): Promise<{ legalPage: Page; openedNewTab: boolean }> {
...
test("saleads_mi_negocio_full_test", async ({ page }, testInfo) => {
  const report = Object.fromEntries(REPORT_FIELDS.map((field) => [field, { status: "FAIL" as ReportStatus, details: "Not executed" }])) as Record<
    ReportField,
    ReportRecord
  >;
