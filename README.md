...
* To run the SaleADS Mi Negocio end-to-end workflow test (URL/domain is passed by configuration, no hardcoded environment):

```
$ SALEADS_LOGIN_URL="https://<current-saleads-environment>/login" mvn -Dtest=io.proleap.saleads.SaleadsMiNegocioWorkflowTest test
```
...
$ mvn -Dtest=io.proleap.saleads.SaleadsMiNegocioWorkflowTest -Dsaleads.loginUrl="https://<env>/login" -Dsaleads.headless=false test

# increase waits for slower environments
$ mvn -Dtest=io.proleap.saleads.SaleadsMiNegocioWorkflowTest -Dsaleads.loginUrl="https://<env>/login" -Dsaleads.timeoutSeconds=60 test
```

Evidence generated by the SaleADS workflow test is stored under:
