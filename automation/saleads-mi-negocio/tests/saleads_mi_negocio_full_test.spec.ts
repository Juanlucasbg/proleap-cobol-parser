...
test("saleads_mi_negocio_full_test", async ({ page }, testInfo) => {
  ...
  const waitForUiToSettle = async (targetPage: Page) => {
    await targetPage.waitForLoadState("domcontentloaded", { timeout: 20_000 }).catch(() => null);
    await targetPage.waitForLoadState("networkidle", { timeout: 10_000 }).catch(() => null);
  ...
  const maybeSelectGoogleAccount = async (authPage: Page) => {
    ...
    const chooseAccountText = authPage.getByText(/Elige una cuenta|Choose an account/i).first();
    if (await chooseAccountText.isVisible().catch(() => false)) {
      const fallbackOption = await pickVisible(
        authPage,
        [authPage.getByText(GOOGLE_ACCOUNT_EMAIL, { exact: true }), authPage.getByText(/@gmail\.com/i)],
  ...
  const openLegalPageAndReturn = async (
    ...
    const legalLink = await pickVisible(
      page,
      [page.getByRole("link", { name: linkPattern }), page.getByRole("button", { name: linkPattern }), page.getByText(linkPattern)],
  ...
  try {
    ...
    await executeStep("Agregar Negocio modal", async () => {
      ...
      const modal = await pickVisible(
        page,
        [page.getByRole("dialog"), page.locator('[role="dialog"]'), page.locator('[class*="modal"]')],
        "Crear Nuevo Negocio modal",
      );

      await pickVisible(page, [modal.getByText(/Crear Nuevo Negocio/i), page.getByText(/Crear Nuevo Negocio/i)], "Modal title");
      ...
      await pickVisible(page, [modal.getByText(/Tienes\s+2\s+de\s+3\s+negocios/i), page.getByText(/Tienes\s+2\s+de\s+3\s+negocios/i)], "Business quota text");
      await pickVisible(page, [modal.getByRole("button", { name: /Cancelar/i }), page.getByText(/Cancelar/i)], "Cancelar button");
      await pickVisible(page, [modal.getByRole("button", { name: /Crear Negocio/i }), page.getByText(/Crear Negocio/i)], "Crear Negocio button");
    ...
    await executeStep("Administrar Negocios view", async () => {
      ...
      const administrarOption = await pickVisible(
        page,
        [page.getByRole("link", { name: /Administrar Negocios/i }), page.getByRole("button", { name: /Administrar Negocios/i }), page.getByText(/Administrar Negocios/i)],
        "Administrar Negocios option",
      );
      await clickAndWait(administrarOption);

      await pickVisible(page, [page.getByText(/Informaci[oó]n General/i)], "Informacion General section");
      await pickVisible(page, [page.getByText(/Detalles de la Cuenta/i)], "Detalles de la Cuenta section");
    ...
    await executeStep("Información General", async () => {
      ...
      await pickVisible(
        page,
        [page.getByText(/Nombre|Usuario|Perfil/i), page.locator("h1, h2, h3, strong, [class*='name']").first()],
        "User name",
      );
      await pickVisible(page, [page.getByText(/BUSINESS PLAN/i)], "Business plan text");
      await pickVisible(page, [page.getByRole("button", { name: /Cambiar Plan/i }), page.getByText(/Cambiar Plan/i)], "Cambiar Plan button");
    });
    ...
    await executeStep("Tus Negocios", async () => {
      ...
      const businessListOrCards = await pickVisible(
        page,
        [page.locator("[data-testid*='business']").first(), page.locator("table tbody tr").first(), page.locator("[class*='business']").first()],
        "Business list",
      );
